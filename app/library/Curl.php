...
class Curl {
    ...
    public function post($url, $data=array()) {
        ...
        if ( isset($data['not_http_build_query']) && true == $data['not_http_build_query'] ) {
            unset($data['not_http_build_query']);
    ...
    public function postXML($url, $xml='') {
        $this->setopt(CURLOPT_URL, $url);
        $this->setopt(CURLOPT_POSTFIELDS, $xml);
        $this->setopt(CURLOPT_RETURNTRANSFER, true);
        $this->setopt(CURLOPT_HTTPHEADER, array('Content-Type: application/xml'));
        $this->_exec();
    }

    public function postFile($url, $data=array(), $size=0) {
    ...
    public function postDelete($url, $data=array()) {
        $this->setopt(CURLOPT_URL, $url);
        $this->setopt(CURLOPT_POST, TRUE);
